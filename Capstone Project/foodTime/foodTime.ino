...
/* Includes ---------------------------------------------------------------- */
#include <SEIS744-Capstone_inferencing.h>
#include <TinyMLShield.h>

byte image[176 * 144 * 2]; // QCIF: 176x144 x 2 bytes per pixel (RGB565)
int bytesPerFrame;

static float features[25600] = {
...
void setup()
{
    ...
    while (!Serial);
    Serial.println("Edge Impulse Food Time Demo");
...
void loop()
{
    ...
    if (clicked) {
      ...
      for (int i = 0; i < bytesPerFrame; i += 2) {
        // 从16位RGB565格式中提取红色、绿色和蓝色分量
        uint16_t rgb565 = (image[i + 1] << 8) | image[i];
        ...
        // 组合扩展后的分量成为24位RGB值
        uint32_t rgb888 = (red8 << 16) | (green8 << 8) | blue8;

        // 存储到features数组
        features[i / 3] = (float)rgb888;
      }
      ...
      const char* label = print_inference_result(result);
